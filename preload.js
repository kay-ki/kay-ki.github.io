...
class Preload extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.audio("snd_music", "assets/catharsis.mp3");
  ...
  create() {
    ...
    let startText = this.add.text(width / 2, height / 2, 'Press Any Key to Start', { 
        fill: '#ffffff',
        fontSize: '32px'
    }).setOrigin(0.5, 0.5);  // Center the text

    this.input.keyboard.once('keydown', () => {
        sound_music = this.sound.add('snd_music', { loop: true, volume: 0 });
        sound_music.play();
