class Title extends Phaser.Scene {
  ...
  create() {
    ...
    WebFont.load({
      google: {
          families: ['Urbanist:600', 'Urbanist:700']
      },
      active: () => {
          this.add.text(10, this.sys.game.config.height - 25, 'M: Toggle Music    E: Toggle Easy Mode', {
              fontFamily: 'Urbanist',
              fontSize: '14px',
              fill: '#ffffff'
          }); 
  ...
  this.input.keyboard.on('keydown-E', () => {
    ...
    console.log('Easy Mode:', easy_mode === 1 ? 'Off' : 'On');
});
    ...
    if (!musicFadedIn) {
      if (!sound_music.isPlaying) {
          sound_music.play({ volume: 0 });
      }

      this.tweens.add({
          targets: sound_music,
          volume: { from: 0, to: 0.05 },
          duration: 2000
      });

      musicFadedIn = true; // Set the flag to indicate fade-in has been done
  }
  ...
  addTitleText() {
    ...
    this.add.text(width * 0.18, height * 0.35, 'Spheralis', {
        fontFamily: 'Urbanist',
        fontSize: '72px',
        fontStyle: '700',
        color: '#ffffff'
